      ...
      <div className="mt-10 grid lg:grid-cols-[1.4fr_1fr] gap-6">
            ...
            >
              ...
              <div className="relative">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="text-[10px] tracking-[0.3em] text-body uppercase">
                      {a.purpose}
                    </div>
                    <div className="font-display text-base heading-text mt-1.5">
                      {a.bank}
                    </div>
                  </div>
                ...
                <div className="mt-5">
                  <div className="text-[10px] tracking-widest text-body/70 uppercase mb-1">
                    Account Name
                  </div>
                  <div className="text-sm heading-text">{a.name}</div>
                </div>

                <div className="mt-3 p-3 rounded-lg bg-accent/5 border border-silver/30">
                  <div className="text-[10px] tracking-widest text-body/70 uppercase mb-1">
                    Account Number
                  </div>
                  <div className="flex items-center justify-between gap-3">
                    <code className="font-display text-base text-body font-bold tracking-wider break-all">
                      {a.number}
                    </code>
                    <button
                      onClick={() => copy(a.number, a.number)}
                      className="shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-accent text-white text-[11px] font-bold px-2.5 py-1 rounded-md"
                    >
                      {copied === a.number ? (
                        <>
                ...
                <div className="mt-3 flex items-center justify-between text-[11px]">
                  <span className="text-body">SWIFT/BIC</span>
                  <code className="text-body">{a.swift}</code>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        >
          <div className="flex items-center gap-2 text-body">
            ...
            <div className="font-display text-lg heading-text">Proof of Payment</div>
          </div>
          <p className="text-xs text-body/80 mt-2">
          ...
          {!submitted ? (
            <>
              ...
              <div
                className="mt-4 border-2 border-dashed border-silver/40 rounded-xl p-5 text-center cursor-pointer hover:border-accent/60 transition"
              ...
              >
                ...
                />
                <Upload className="w-6 h-6 text-body mx-auto mb-2" />
                <div className="text-sm text-body/90">
              ...
              <button
                ...
                className="mt-5 w-full py-3 rounded-full btn-primary inline-flex items-center justify-center gap-2"
              >
            ...
            >
              <div className="w-16 h-16 mx-auto rounded-full bg-accent grid place-items-center text-white mb-3">
                <Check className="w-8 h-8 text-white" />
              </div>
              <div className="font-display text-xl heading-text">Thank you!</div>
              ...
              <button
                ...
                className="mt-5 px-5 py-2 rounded-full btn-primary text-sm"
              >
                Submit another
              </button>
            </motion.div>
...
function Field({
  ...
  return (
    <label className="block">
      <div className="text-[10px] tracking-widest text-body/80 uppercase mb-1.5">
        {label}
      </div>
      <input
        {...props}
        className="w-full bg-white border border-silver/30 rounded-lg px-3 py-2 text-sm text-body placeholder:text-body/40 focus:outline-none focus:border-accent/50"
      />
    </label>
  );
}
...
function Select({ label, options }: { label: string; options: string[] }) {
  return (
    <label className="block">
      ...
      <select className="w-full bg-white border border-silver/30 rounded-lg px-3 py-2 text-sm text-body focus:outline-none focus:border-accent/50">
        {options.map((o) => (
