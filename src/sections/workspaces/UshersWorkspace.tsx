  ...
  if (submitted) {
    return (
      <div className="max-w-sm mx-auto px-5 py-10 pt-20 text-center">
        <div className="w-20 h-20 mx-auto rounded-full bg-accent grid place-items-center text-white mb-4">
          <CheckCircle2 className="w-10 h-10 text-body" />
        </div>
        <div className="font-display text-2xl heading-text">Submitted!</div>
        <p className="text-sm text-body mt-2">Tonight's count sent to HQ. First-timers will receive welcome SMS.</p>
        <button onClick={() => { setSubmitted(false); setCount({ men: 0, women: 0, children: 0, firstTimers: 0 }); }} className="mt-6 px-6 py-3 rounded-full btn-primary text-sm">
          New Service Count
        </button>
      </div>
    );
    ...
    <div className="max-w-lg mx-auto px-5 py-10 pt-20">
      ...
      <div className="relative rounded-[2.5rem] p-2 bg-gradient-to-b from-accent-dark via-accent-dark to-accent-dark border border-silver/30 shadow-2xl">
        <div className="rounded-[2.2rem] glass-dark p-5">
          <div className="flex items-center justify-between text-[11px] text-body mb-4">
            <Smartphone className="w-3 h-3" />
            <span className="text-accent/80 tracking-wider text-[10px] uppercase">End-Count</span>
            <span className="text-body">Lagos HQ</span>
          </div>
          ...
          <div className="mt-4 p-3 rounded-xl bg-gradient-to-r from-accent to-accent-dark border border-silver/30 flex items-center justify-between">
            <div className="text-[11px] tracking-widest text-accent/80 uppercase">Total</div>
            <div className="font-display text-3xl heading-text">{total}</div>
          </div>

          <button onClick={submit} className="mt-4 w-full py-3 rounded-full btn-primary inline-flex items-center justify-center gap-2">
      ...
      <div className="mt-6 glass rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[9px] tracking-widest text-accent">AUTO-SMS</span>
        </div>
        <div className="flex flex-col gap-2">
          <input value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Full name" className="bg-white border border-silver/30 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-silver/50" />
          <input value={draft.phone} onChange={(e) => setDraft({ ...draft, phone: e.target.value })} placeholder="Phone / WhatsApp" className="bg-white border border-silver/30 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-silver/50" />
          <div className="flex gap-2">
            <input value={draft.invitedBy} onChange={(e) => setDraft({ ...draft, invitedBy: e.target.value })} placeholder="Invited by" className="flex-1 bg-white border border-silver/30 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-silver/50" />
            <button onClick={addFT} className="px-4 py-2.5 rounded-lg btn-primary text-sm"><Plus className="w-4 h-4" /></button>
          </div>
        </div>
          ...
          {firstTimers.map((f, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-black/30 border border-silver/10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-body">{f.phone}</div>
              </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {[
          ...
          <div key={s.label} className="glass rounded-xl p-3 text-center">
            ...
            <div className="font-display text-base text-body mt-1">{s.value}</div>
            <div className="text-[9px] text-body uppercase tracking-widest">{s.label}</div>
          </div>
        ))}
      </div>
    </div>
