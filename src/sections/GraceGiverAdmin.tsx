...
export default function GraceGiverAdmin() {
  ...
  const Input = ({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) => (
    <div>
      <label className="text-[10px] tracking-widest text-accent/80 uppercase mb-1 block">{label}</label>
      <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full bg-white border border-silver/30 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-accent/50" />
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto px-5 lg:px-8 py-10">
      <div className="pt-20">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 text-white/80 text-[11px] tracking-[0.4em] uppercase mb-3">
              <Heart className="w-4 h-4" /> Admin
            </div>
            <h1 className="font-display text-4xl md:text-5xl heading-text">Grace Giver Accounts</h1>
            <p className="mt-3 text-body">Manage bank accounts displayed on the Grace-Giver Hub page.</p>
          </div>
        ...
        {accounts.map((a, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="font-display text-base text-body inline-flex items-center gap-2">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: a.color || "#800000" }} />
                Account #{i + 1}
              </div>
              {accounts.length > 1 && (
                <button onClick={() => removeAccount(i)} className="p-2 rounded-lg hover:bg-white/5 text-accent">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                <label className="text-[10px] tracking-widest text-accent/80 uppercase mb-1 block">Color</label>
                <input type="color" value={a.color || "#800000"} onChange={(e) => updateAccount(i, "color", e.target.value)} className="w-full h-[42px] rounded-xl bg-white border border-silver/30 cursor-pointer" />
              </div>
