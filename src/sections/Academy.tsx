  ...
  if (courses.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-10">
        ...
        />
        <div className="mt-12 text-center py-16 text-body text-sm">Loading courses...</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 lg:px-8 py-10">
      ...
      <div className="mt-8 grid sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className="glass rounded-xl p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-accent/40 grid place-items-center">
              <s.icon className="w-4 h-4 text-accent" />
            </div>
            <div>
              <div className="font-display text-xl heading-text">{s.value}</div>
              <div className="text-[11px] uppercase tracking-widest text-body">{s.label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-8 grid lg:grid-cols-[1.4fr_1fr] gap-6">
            ...
            >
              ...
              <div className="relative">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="text-[10px] tracking-widest text-accent/70 uppercase">{c.code}</div>
                    <div className="font-display text-base text-body mt-1">{c.title}</div>
                    <div className="text-[11px] text-body mt-0.5">{c.instructor || 'HEC Faculty'}</div>
                  </div>
                  {c.badge ? (
                    <span className="text-[9px] px-2 py-0.5 rounded-full bg-accent text-white font-bold tracking-widest">{c.badge}</span>
                  ) : c.is_locked ? (
                    <Lock className="w-4 h-4 text-body" />
                  ) : null}
                </div>
                <div className="mt-4 flex items-center justify-between text-[11px] text-body">
        ...
        >
          <div className="text-[10px] tracking-widest text-accent/80 uppercase">{sel.code}</div>
          <div className="font-display text-2xl text-body mt-1">{sel.title}</div>
          <div className="text-xs text-body mt-1">{sel.instructor || 'HEC Faculty'}</div>
          {sel.description && (
            <p className="mt-3 text-xs text-body leading-relaxed">{sel.description}</p>
          )}

          <div className="mt-5">
            <div className="text-[11px] tracking-widest text-accent/80 uppercase mb-2">Modules</div>
            <div className="space-y-1.5">
              {modules.length === 0 && (
                <div className="text-xs text-body text-center py-4">No modules loaded</div>
              )}
              {modules.map((m) => (
          ...
          {sel.is_locked ? (
            <div className="mt-6 w-full py-3 rounded-full glass text-sm text-center text-body">
              <Lock className="w-4 h-4 inline mr-1" /> Locked — Complete previous courses
            </div>
          ) : (
            <button className="mt-6 w-full py-3 rounded-full btn-primary inline-flex items-center justify-center gap-2">
