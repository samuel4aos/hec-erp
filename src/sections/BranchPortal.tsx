    ...
    <div className="max-w-7xl mx-auto px-5 lg:px-8 py-10">
        ...
        {(Object.keys(roleConfig) as Role[]).map((r) => {
          ...
          return (
            <button key={r} onClick={() => setRole(r)}
              className={`text-left glass rounded-2xl p-4 transition border ${active ? "border-gold ring-1 ring-gold/40" : "border-transparent"}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="font-display text-sm text-parchment truncate">{c.name}</div>
                </div>
              </div>
            </button>
          );
      ...
      <div className="mt-6 glass-dark rounded-2xl p-5 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <div className="font-display text-xl text-parchment mt-1">{roleConfig[role].name}</div>
          <div className="text-xs text-parchment/65">{roleConfig[role].subtitle}</div>
        </div>
        <div className="flex items-center gap-2 text-[11px] text-verdant-light">
    ...
    <div className="grid lg:grid-cols-3 gap-5">
      ...
      <div className="lg:col-span-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          <div key={s.l} className="glass rounded-xl p-3">
            ...
            <div className="font-display text-lg gold-text">{s.v}</div>
            <div className="text-[9px] uppercase tracking-widest text-parchment/55">{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div className="lg:col-span-2 glass rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div className="font-display text-lg gold-text">Global Giving Feed</div>
        </div>
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {data.recentGiving.length === 0 && <div className="text-xs text-parchment/50 text-center py-8">No giving records yet</div>}
          {data.recentGiving.map((g: any, i: number) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-black/30 border border-gold/10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-parchment/55">{g.full_name} · {g.currency} · {g.status}</div>
              </div>
      ...
      <div className="glass rounded-2xl p-5">
        <div className="font-display text-base text-parchment mb-4">Branch Health</div>
        {data.branchesList.map((b: any) => (
          <div key={b.id} className="mb-3 last:mb-0">
            <div className="flex items-center justify-between text-xs mb-1">
              <span className="text-parchment/85 truncate">{b.name}</span>
              <span className="text-gold font-display">{b.code}</span>
            </div>
          </div>
        ))}
        {data.branchesList.length === 0 && <div className="text-xs text-parchment/50 text-center py-4">No branches</div>}
      </div>

      {/* Bookstore CMS Recent */}
      <div className="lg:col-span-3 glass rounded-2xl p-5">
        <div className="font-display text-base text-parchment mb-4">Bookstore Products · Recent</div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {data.products.length === 0 && <div className="col-span-full text-xs text-parchment/50 text-center py-4">No products</div>}
          {data.products.map((p: any) => (
            <div key={p.id} className="glass rounded-xl p-3 border border-gold/15">
              <div className="text-sm text-parchment">{p.title}</div>
              <div className="text-[11px] text-gold/80 mt-1">${Number(p.price).toFixed(2)} · {p.status}</div>
            </div>
          ))}
        </div>
      </div>
...
function PastorView() {
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-5">
      {/* Care Alerts */}
      <div className="lg:col-span-2 glass rounded-2xl p-5">
        <div className="font-display text-base text-parchment mb-4 flex items-center gap-2">
          <AlertCircle className="w-4 h-4 text-maroon-light" /> Care Alerts
          {missed.length > 0 && <span className="text-[10px] bg-red-900/40 text-red-400 px-2 py-0.5 rounded-full">{missed.length}</span>}
        </div>
        {missed.length === 0 && <div className="text-xs text-parchment/50 text-center py-4">No care alerts — members are attending regularly</div>}
        {missed.map((m: any, i: number) => (
          <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-red-900/20 border border-red-900/40 mb-2">
            <div className="flex-1 min-w-0">
              <div className="text-sm text-parchment">{m.member?.first_name} {m.member?.last_name}</div>
              <div className="text-[11px] text-parchment/55">{m.member?.phone} · Missed {m.missed_streak} Sundays</div>
            </div>
      ...
      <div className="space-y-5">
        ...
        <div className="glass rounded-2xl p-5">
          <div className="font-display text-base text-parchment mb-3">Discipleship Pipeline</div>
          {[
            ...
            <div key={s.stage} className="mb-2 last:mb-0">
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-parchment/85">{s.stage}</span>
                <span className="text-gold font-display">{s.n}</span>
              </div>
        ...
        {/* Counselling */}
        <div className="glass rounded-2xl p-5">
          <div className="font-display text-base text-parchment mb-3">Open Counselling</div>
          {counselling.length === 0 && <div className="text-xs text-parchment/50 text-center py-3">No open sessions</div>}
          {counselling.slice(0, 3).map((c: any) => (
            <div key={c.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-black/25 mb-1.5">
              <div className="text-sm text-parchment flex-1 min-w-0 truncate">{c.member_name}</div>
              <div className="text-[10px] text-parchment/50">{c.visit_type}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Preaching Log */}
      <div className="lg:col-span-3 glass rounded-2xl p-5">
        <div className="font-display text-base text-parchment mb-3">First-Timer Follow-ups</div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead><tr className="text-parchment/50 uppercase tracking-widest">
            ...
            <tbody>
              {firstTimers.slice(0, 5).map((f: any) => (
                <tr key={f.id} className="border-t border-gold/10">
                  <td className="py-2 pr-4 text-parchment">{f.full_name}</td>
                  <td className="py-2 pr-4 text-parchment/70">{f.phone}</td>
                  <td className="py-2 pr-4"><span className="text-gold">{f.follow_up_status}</span></td>
                  <td className="py-2 text-parchment/50">{new Date(f.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-5">
      <div className="lg:col-span-2 space-y-5">
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={s.l} className="glass rounded-xl p-4">
              <div className="text-[10px] uppercase tracking-widest text-parchment/55">{s.l}</div>
        ...
        <div className="glass rounded-2xl p-5">
          <div className="font-display text-base text-parchment mb-3">Pending Verifications</div>
          {pendingVerify.length === 0 && <div className="text-xs text-parchment/50 text-center py-4">All caught up</div>}
          {pendingVerify.slice(0, 5).map((g: any) => (
            <div key={g.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-black/30 border border-gold/10 mb-2">
              <div className="flex-1 min-w-0">
                <div className="text-sm text-parchment">{g.full_name}</div>
                <div className="text-[11px] text-parchment/55">{g.purpose} · ${Number(g.amount).toLocaleString()}</div>
              </div>
              <button onClick={() => api.verifyGiving(g.id).then(load)} className="px-3 py-1 rounded-md btn-gold text-[11px]">Verify</button>
            </div>
          ))}
        </div>
        ...
        <div className="glass rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="font-display text-base text-parchment">Expense Ledger</div>
          </div>
          <div className="grid grid-cols-4 gap-2 mb-3">
            <input value={desc} onChange={e => setDesc(e.target.value)} placeholder="Description" className="col-span-2 bg-black/40 border border-gold/15 rounded-lg px-3 py-2 text-sm" />
            <input value={amount} onChange={e => setAmount(e.target.value)} placeholder="Amount" type="number" className="bg-black/40 border border-gold/15 rounded-lg px-3 py-2 text-sm" />
            <button onClick={addExpense} className="btn-gold rounded-lg text-xs">Add</button>
          </div>
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {expenses.length === 0 && <div className="text-xs text-parchment/50 text-center py-4">No expenses</div>}
            {expenses.map((e: any) => (
              <div key={e.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-black/30 border border-gold/10">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-parchment/55">{e.category} · {new Date(e.created_at).toLocaleDateString()}</div>
                </div>
      ...
      <div className="space-y-5">
        <div className="glass rounded-2xl p-5">
          <div className="font-display text-base text-parchment mb-1">Remit to HQ</div>
          <p className="text-xs text-parchment/60 mb-4">Submit branch remittance for HQ reconciliation.</p>
          <div className="space-y-3">
            <input value={remitAmount} onChange={e => setRemitAmount(e.target.value)} placeholder="Amount" type="number" className="w-full bg-black/40 border border-gold/15 rounded-lg px-3 py-2 text-sm" />
            <input value={remitPeriod} onChange={e => setRemitPeriod(e.target.value)} placeholder="Period (e.g. 2026-05)" className="w-full bg-black/40 border border-gold/15 rounded-lg px-3 py-2 text-sm" />
            <textarea value={remitNotes} onChange={e => setRemitNotes(e.target.value)} placeholder="Notes" rows={2} className="w-full bg-black/40 border border-gold/15 rounded-lg px-3 py-2 text-sm resize-none" />
            <button onClick={submitRemittance} className="w-full py-3 rounded-full btn-gold inline-flex items-center justify-center gap-2">
        ...
        <div className="glass rounded-2xl p-5">
          <div className="font-display text-base text-parchment mb-3">Remittance History</div>
          {remittances.length === 0 && <div className="text-xs text-parchment/50 text-center py-3">No remittances</div>}
          {remittances.map((r: any) => (
            <div key={r.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-black/25 mb-1.5">
              <div className="text-sm text-parchment flex-1">${Number(r.amount).toLocaleString()}</div>
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${r.status === 'reconciled' ? 'bg-verdant/20 text-verdant-light' : 'bg-gold/20 text-gold'}`}>{r.status}</span>
            </div>
          ))}
        </div>
      </div>
...
function UshersView() {
  ...
  if (submitted) return (
    <div className="text-center py-16">
      <CheckCircle2 className="w-16 h-16 text-verdant-light mx-auto mb-4" />
      <div className="font-display text-2xl gold-text">Service Count Submitted</div>
      <p className="text-parchment/65 mt-2 text-sm">HQ has been notified.</p>
      <button onClick={() => { setSubmitted(false); setCount({ men: 0, women: 0, children: 0, firstTimers: 0 }); setFtList([]); }} className="mt-6 px-6 py-3 rounded-full btn-gold">New Service Count</button>
    </div>
  );
    ...
    <div className="grid lg:grid-cols-[420px_1fr] gap-5">
      <div className="mx-auto lg:mx-0 w-full max-w-sm">
        <div className="relative rounded-[2.5rem] p-2 bg-gradient-to-b from-ink via-maroon-dark to-ink border border-gold/30 shadow-2xl">
          <div className="rounded-[2.2rem] glass-dark p-5">
            <div className="flex items-center justify-between text-[11px] text-parchment/60 mb-3">
              <span>Service Count</span>
              <select value={serviceType} onChange={e => setServiceType(e.target.value)}
                className="bg-transparent text-[11px] text-gold border border-gold/20 rounded px-2 py-0.5">
                <option value="Sunday">Sunday</option><option value="Midweek">Midweek</option><option value="Crusade">Crusade</option>
              </select>
            </div>
            <div className="text-[10px] tracking-widest text-gold/80 uppercase">Service End-Count</div>
            <div className="font-display text-2xl text-parchment mt-1">{serviceType} Tally</div>
            ...
            <div className="mt-4 p-3 rounded-xl bg-gradient-to-r from-maroon to-maroon-dark border border-gold/30 flex items-center justify-between">
              <div className="text-[11px] tracking-widest text-gold/80 uppercase">Total in Sanctuary</div>
              <div className="font-display text-3xl gold-text">{total}</div>
            </div>
            <button onClick={handleSubmit} className="mt-4 w-full py-3 rounded-full btn-gold inline-flex items-center justify-center gap-2">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
          {[
            ...
            <div key={s.l} className="glass rounded-xl p-3">
              <s.ic className="w-4 h-4 text-gold mx-auto" />
              <div className="font-display text-base text-parchment mt-1">{s.v}</div>
              <div className="text-[10px] text-parchment/55 uppercase tracking-widest">{s.l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="glass rounded-2xl p-5">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[10px] tracking-widest text-verdant-light">AUTO-WELCOME</span>
        </div>
        <p className="text-xs text-parchment/65 mb-4">Each entry triggers an automated welcome SMS and follow-up.</p>
        <div className="grid sm:grid-cols-3 gap-2 mb-3">
          <input value={draft.name} onChange={e => setDraft({...draft, name: e.target.value})} placeholder="Full name" className="bg-black/40 border border-gold/15 rounded-lg px-3 py-2.5 text-sm" />
          <input value={draft.phone} onChange={e => setDraft({...draft, phone: e.target.value})} placeholder="Phone / WhatsApp" className="bg-black/40 border border-gold/15 rounded-lg px-3 py-2.5 text-sm" />
          <input value={draft.invitedBy} onChange={e => setDraft({...draft, invitedBy: e.target.value})} placeholder="Invited by" className="bg-black/40 border border-gold/15 rounded-lg px-3 py-2.5 text-sm" />
        </div>
        <button onClick={addFT} className="px-4 py-2 rounded-full btn-gold text-sm inline-flex items-center gap-2"><Plus className="w-4 h-4" /> Add first-timer</button>
        <div className="mt-5 space-y-2 max-h-80 overflow-y-auto">
          {ftList.map((f, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-black/30 border border-gold/10">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-gold to-maroon grid place-items-center text-[11px] font-display text-ink">
                {f.name.split(" ").map(n => n[0]).slice(0,2).join("")}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm text-parchment truncate">{f.name}</div>
                <div className="text-[11px] text-parchment/55">{f.phone} {f.invitedBy && `· by ${f.invitedBy}`}</div>
              </div>
            </div>
          ))}
        </div>
...
function Field({ label, rows = 1, ...props }: { label: string; rows?: number } & TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <label className="block">
      <div className="text-[10px] tracking-widest text-parchment/55 uppercase mb-1.5">{label}</div>
      <textarea {...props} rows={rows} className="w-full bg-black/40 border border-gold/15 rounded-lg px-3 py-2 text-sm text-parchment focus:outline-none focus:border-gold/50 resize-none" />
    </label>
  );
}
