    ...
    <div className="max-w-7xl mx-auto px-5 lg:px-8 py-10">
      <div className="pt-20">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="font-display text-4xl md:text-5xl gold-text">First-Timer Pipeline</h1>
      ...
      <div className="mt-6 grid grid-cols-4 gap-3">
        <div className="glass rounded-xl p-4">
          <div className="font-display text-2xl gold-text">{total}</div>
          <div className="text-[10px] uppercase tracking-widest text-parchment/55">Total Pipeline</div>
        </div>
        {STATUS_ORDER.map((s) => (
          <div key={s} className="glass rounded-xl p-4">
            ...
            <div className="text-[10px] uppercase tracking-widest text-parchment/55">{STATUS_LABELS[s]}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 relative max-w-md">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, phone, or branch..." className="w-full bg-black/40 border border-gold/15 rounded-full pl-10 pr-4 py-2.5 text-sm text-parchment focus:outline-none focus:border-gold/50" />
      </div>
          ...
          <div key={col.status} className="glass-dark rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="inline-flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-parchment/80">{col.label}</span>
              </div>
              <span className="text-[10px] font-display text-parchment/50">{col.count}</span>
            </div>
            <div className="space-y-2 min-h-[200px] max-h-[600px] overflow-y-auto">
              {col.items.length === 0 && (
                <div className="text-[11px] text-parchment/40 text-center py-6">None</div>
              )}
              {col.items.map((ft) => (
                <motion.div
                  key={ft.id}
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-sm text-parchment font-medium truncate">{ft.full_name}</div>
                      {ft.phone && <div className="text-[10px] text-parchment/50 truncate inline-flex items-center gap-1"><Phone className="w-2.5 h-2.5" /> {ft.phone}</div>}
                      {ft.invited_by && <div className="text-[9px] text-parchment/40">by {ft.invited_by}</div>}
                      <div className="text-[9px] text-parchment/40 mt-1">{ft.branch_name} · {new Date(ft.service_date).toLocaleDateString()}</div>
                      {ft.welcome_sms_sent && <div className="text-[9px] text-verdant-light mt-0.5">✓ SMS sent</div>}
                    </div>
                  </div>
                  {col.status !== "enrolled" && (
                    <button
                  ...
                  {col.status === "enrolled" && (
                    <div className="mt-2 text-[9px] text-verdant-light text-center inline-flex items-center justify-center gap-1 w-full py-1">
