  ...
  return (
    <div className="max-w-7xl mx-auto px-5 lg:px-8 py-10">
      <div className="pt-20">
        ...
        <h1 className="font-display text-4xl md:text-5xl heading-text">Event Registrations</h1>
        <p className="mt-3 text-body max-w-2xl">View and check in attendees for all events.</p>
      ...
      <div className="mt-6 relative max-w-md">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, event, or email..." className="w-full bg-white border border-silver/30 rounded-full pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-accent/50" />
      </div>

      <div className="mt-6 space-y-2">
        {filtered.length === 0 && <div className="text-center py-12 text-body text-sm">No registrations found.</div>}
        {filtered.map((reg, i) => (
          <motion.div
            key={reg.id}
          ...
          >
            <button onClick={() => toggleCheckin(reg.id)} className="shrink-0">
              {reg.checked_in ? (
                ...
                <Circle className="w-6 h-6 text-body" />
              )}
            </button>
            <div className="flex-1 min-w-0">
              <div className="font-display text-base text-body">{reg.full_name}</div>
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-body mt-1">
                {reg.event_title && <span className="inline-flex items-center gap-1"><ExternalLink className="w-3 h-3 text-accent" /> {reg.event_title}</span>}
                {reg.email && <span>{reg.email}</span>}
                {reg.phone && <span>{reg.phone}</span>}
              </div>
            </div>
            <div className="text-right shrink-0">
              <div className={`text-[10px] font-bold tracking-wider ${reg.checked_in ? "text-accent" : "text-accent/60"}`}>
                {reg.checked_in ? "CHECKED IN" : "NOT CHECKED IN"}
              </div>
              <div className="text-[10px] text-body mt-0.5">{new Date(reg.registered_at).toLocaleDateString()}</div>
            </div>
          </motion.div>
        ))}
      </div>
